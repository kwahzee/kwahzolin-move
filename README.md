...
2 oscillators, triangle-shaped, circling each other. osc2 is the clock. every upswing pushes a new bit into the 8-bit shift register from osc1's sign. the register value feeds back into both oscillator frequencies and into the filter's cutoff. three independent lfos. each can  target any of the 8 knob parameters at audio-rate resolution. an optional distortion and/or stereo spring-like reverberator can be applied after the filter. 
